...
export class Tab1Component implements OnInit {
  ...
  async ngOnInit() {
    try {
      ...
      // BAR CHART
      // Preprocess
      let popularInterventions: { [key: string]: any }[] =
        this.preprocessingService.popularInterventions;
      // Viz
      ...
      const listeDeputes44: { [key: string]: any }[] =
        this.preprocessingService.listeDeputes44;
      ...
      this.addingStatActiveMPs(
        this.preprocessingService.deputesLegislatures,
        this.preprocessingService.debats
      );

      // KEY VALUES with listedeputes.csv : number of changes since beginning legislature
      const changesLegislature44: { [key: string]: any }[] =
        this.preprocessingService.changesLegislature44;
  ...
  async createWaffleGraph(): Promise<void> {
    ...
    await waffle.drawWaffleLegend(
      partyColorScale,
      this.preprocessingService.getScale()
    );
  }
  /**
  ...
  waffleLookNice(nbRow: number, nbBlocCol = 8, nbBlocRow = 5): void {
    ...
    for (let row = 0; row < nbRow; row++) {
      for (let i = 0; i < nbBlocCol; i++) {
        d3.selectAll(
          "rect[col='" + String(i) + "'][row='" + String(row) + "']"
  ...
  addingStatIncreaseWomen(): void {
    ...
    const statSpan3: HTMLSpanElement | null = document.getElementById(
      'stat3'
    ) as HTMLSpanElement;
    if (statSpan3) {
  ...
  addingStatActiveMPs(
    ...
    const statSpan1: HTMLSpanElement | null = document.getElementById(
      'stat1'
    ) as HTMLSpanElement;
    if (statSpan1) {
      ...
      const percentActiveMPs: number =
        this.preprocessingService.percentageActiveMP;
      // Inject the value into the <span> element
  ...
  addingStatChangeLegislature(changesLegislature44: { [key: string]: any }[]) {
    ...
    const statSpan2: HTMLSpanElement | null = document.getElementById(
      'stat2'
    ) as HTMLSpanElement;
    if (statSpan2) {
      ...
      if (changesLegislature44.length == 0) {
        ...
        // if only one MP changed his party, we diplay his name
        innerStatSpan.textContent = '1'; // this text will be bold
        const textAfter: Text = document.createTextNode(
          'député (' + changesLegislature44[0]['nom'] + ') a'

...
export class Tab3Component implements OnInit {
  ...
  ngOnInit(): void {
    d3.csv('./assets/data/debatsCommunesNotext.csv', d3.autoType).then(
      (data) => {
        this.data = data;
        // 44ème législature
        ...
        let Ymax =
          preprocessTab3.getMaxCharCounts(groupedArraysByType) / 1000000;
  ...
  updateView(): void {
    //importer data une fois seulement à place de le refaire à chaque changement
    this.ngOnInit();
  }
        ...
        tooltip
          ...
          .html(
            translatePretty(d['KeyElement']) +
              ' en ' +
              translateDate(xvalue) +
              ':<br> - ' +
              d['Count'] +
              ' interventions <br> - ' +
              d['CharCount'] +
              ' caractères dans ces interventions'
          );
